function [acertos erros] = crossvalidationnew(t0,tinc,tq,nf)
    nimgs = 1700; %Total de imagens
    ncol = 700;
    nlag=1000;
    t = t0:tinc:tq;
    ncar = length(t)*3; %qtd caracteristicas = tam conj teste
    ...
    for i=1:niter
        for j=1:qtdset
            ...
            for p=1:10
                p
                [x1 x2 u1 u2] = normalizanew(p,t,setsc(i,j,:),setsl(i,j,:));
                c1 = cov(x1);
                c2 = cov(x2);
                [l1 c] = size(x1);
                [l2 c] = size(x2);
